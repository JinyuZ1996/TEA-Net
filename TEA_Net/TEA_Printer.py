...
def print_config():
    print("Model Configs: \n"
          ...
          "Alpha: {}, num_heads: {}, dim_coefficient: {}. ".format
          (args.fast_running, args.lr_A, args.lr_B, args.epochs, args.embedding_size,
           args.alpha, args.num_heads, args.dim_coefficient))


def print_rec_message(epoch, rec_loss_A, rec_loss_B, rec_pre_time):
    print('Epoch {} - Training Loss A {:.5f}, Loss B {:.5f} - Training time: {:.3}'.
          format(epoch, rec_loss_A, rec_loss_B, rec_pre_time))
    logging.info('Epoch {} - Training Loss A {:.5f}, Loss B {:.5f} - Training time: {:.3}'.
          format(epoch, rec_loss_A, rec_loss_B, rec_pre_time))
